...
//  COMPLETED (18) Create a String array containing the names of the desired data columns from our ContentProvider
    private static final String[] WEATHER_DETAIL_PROJECTION = new String[] {
...
//  COMPLETED (19) Create constant int values representing each column name's position above
    ...
    public static final int COLUMN_POS_MAX_TEMP = 2;
    public static final int COLUMN_POS_MIN_TEMP = 3;
    public static final int COLUMN_POS_HUMIDITY = 4;
    public static final int COLUMN_POS_PRESSURE = 5;
    public static final int COLUMN_POS_WIND_SPEED = 6;
    public static final int COLUMN_POS_DEGREE = 7;
//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    private final int WETHAER_DETAIL_LOADER_ID = 84;
...
//      COMPLETED (13) Find each of the TextViews by ID
        ...
        mWeatherDescription = (TextView) findViewById(R.id.tv_display_desc);
        mWeatherTempHigh = (TextView) findViewById(R.id.tv_display_temp_high);
        mWeatherTempLow = (TextView) findViewById(R.id.tv_display_temp_low);
...
//      COMPLETED (17) Throw a NullPointerException if that URI is null
        if(mUri == null) {
            throw new NullPointerException("Bad uri");
        }
//      COMPLETED (35) Initialize the loader for DetailActivity
        getSupportLoaderManager().initLoader(WETHAER_DETAIL_LOADER_ID, null, this);
    }
        ...
        switch (id) {
            case WETHAER_DETAIL_LOADER_ID:
                return new CursorLoader(this,
                        ...
                        null);
            default:
                throw new UnsupportedOperationException("Unknown leader -> " + id);
        }
    }
...
//      COMPLETED (26) Display a readable data string
            String date = SunshineDateUtils.getFriendlyDateString(this,data.getLong(COLUMN_POS_DATE),true);
...
//      COMPLETED (27) Display the weather description (using SunshineWeatherUtils)
            String weatherDesc = SunshineWeatherUtils.getStringForWeatherCondition(this,
                    data.getInt(COLUMN_POS_WEATHER_ID));
            mWeatherDescription.setText(weatherDesc);
//      COMPLETED (28) Display the high temperature
            String tempHigh = SunshineWeatherUtils.formatTemperature(this,
                    data.getDouble(COLUMN_POS_MAX_TEMP));
            mWeatherTempHigh.setText(tempHigh);
//      COMPLETED (29) Display the low temperature
            String tempLow = SunshineWeatherUtils.formatTemperature(this,
                    data.getDouble(COLUMN_POS_MIN_TEMP));
            mWeatherTempLow.setText(tempLow);
//      COMPLETED (30) Display the humidity
            float humidity = data.getFloat(COLUMN_POS_HUMIDITY);
